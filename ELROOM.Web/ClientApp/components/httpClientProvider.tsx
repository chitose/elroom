...
export class HttpClientProvider extends React.Component<any, HttpClientProviderState> implements HttpClientAPI {
  ...
  async fetchData<T>(url: string, req?: any): Promise<T | BusinessExceptionResponse> {
    ...
    try {
      ...
      this.setRequestError(this.i18n.t("common:message.network_error"));
      throw ex;
    }
  ...
  setRequestError(msg: string | string[]) {
    ...
    this.systemAPI.alert(this.i18n.t("common:label.failed_request_dialog"), errorMsg);
  }
  ...
  handleKnownErrors(response: { status: number;[key: string]: any }) {
    if (response.status >= 500) {
      this.setRequestError(this.i18n.t('common:message.server_error'));
      return;
    }

...
import { DialogContentComponent, DialogContentAPI } from './dialogContentProvider';
...
export class SystemProvider extends React.Component<SystemProviderProps, SystemProviderState> implements SystemAPI {
    ...
    renderConfirmActions(ci: ConfirmInfo): JSX.Element[] {
        return [
            <FlatButton label={this.i18n.t(ci.noLabel || "common:button.no")} onTouchTap={
                () => { ci.promiseResolve(false); this.closeConfirm(ci) } } keyboardFocused={true} />,
            <FlatButton label={this.i18n.t(ci.yesLabel || "common:button.yes")} onTouchTap={() => { ci.promiseResolve(true); this.closeConfirm(ci) } }/>
        ];
    }

    renderAlertActions(ai: AlertInfo): JSX.Element[] {
        return [
            <FlatButton label={this.i18n.t(ai.okLabel || "common:button.OK")} onTouchTap={() => { ai.promiseResolve(true); this.closeAlert(ai) } }/>
        ];
    }
        ...
        return <HttpClientProvider>
            <ProfileProvider>
                ...
                {this.state.dialogData.map(di => {
                  return <Dialog autoScrollBodyContent={true} key={`dialog-${di.id}`} className={"dialog " + (di.cssClass || "")} contentStyle={di.style || {}} title={di.title} modal={true} open={di.open}>

...
export interface Validator {
  dependencies: string[];
  isValid(value: any, context: DictionaryType<any>): DictionaryType<ValidationResult>;
}
...
class BaseValidator implements Validator {
  ...
  protected getMessageOpt(value: any): DictionaryType<any> {
    return { value: String(value) };
  }

  isValidInt(value: any, context: DictionaryType<any>): boolean {
    return false;
  }

  isValid(value: any, context: DictionaryType<any>): DictionaryType<ValidationResult> {
    const errorMsg: DictionaryType<ValidationResult> = {};
...
class RequiredValidator extends BaseValidator {
  ...
  isValidInt(value: string, context: DictionaryType<any>): boolean {
...
class PasswordRestrict extends BaseValidator {
  ...
  isValid(value: string, context: DictionaryType<any>): DictionaryType<ValidationResult> {
    ...
    if (passOpts.requireDigit && !/(.+)?\d(.+)?/gi.test(value)) {
      return { password: { message: i18n.t("validation:password.require_digit") } };
    }
    if (passOpts.requireLowercase && !/(.+)?[a-z](.+)?/g.test(value)) {
      return { password: { message: i18n.t("validation:password.require_lowercase") } };
    }
    if (passOpts.requireUppercase && !/(.+)?[A-Z](.+)?/g.test(value)) {
      return { password: { message: i18n.t("validation:password.require_uppercase") } };
    }
    if (passOpts.requireNonAlphanumeric && !/(.+)?[^a-zA-Z\d](.+)?/g.test(value)) {
      return { password: { message: i18n.t("validation:password.require_non_alphanumeric") } };
    }
    return null;
  }
}

${
  ...
  IEnumerable<Type> ClassDependencies(Class @class) => Dependencies(@class).Where(t => !t.IsEnum);
...
$Classes(:ApiController)[$ClassDependencies[
...
$Methods[$HasParameter[export function $name(api: HttpClientAPI,$Parameters(p => !p.Attributes.Any(a => a.Name == "FromServices"))[$name: $ParamType][, ]) {
  return api.http<$ReturnType>(`$UrlCore`, { method: '$HttpMethod' $JsonBody });
}]
